...
跳过写入并保留旧预测批次的有限值，而首次只保留少量历史批次的本服无法重建该值。
因此仅当模型为 EC9、周期为小时、官方值为有限 JSON 数值且本地值严格为 `null` 时，
验证继续执行。每个例外都会以
`official_rolling_value_over_local_nan` 写入不可变点位回执，包含点位、变量、时间、
官方值、本地值和本地批次，并在报告中单独计数。数值对数值、官方 `null` 对本地有限值、
其他模型以及日字段的任何差异仍立即失败。本规则不等同于通用的缺失值豁免。
