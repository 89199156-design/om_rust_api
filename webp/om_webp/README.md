...
The production inventory renders model-specific GFS, ECMWF and CAMS layers for the configured first 121 forecast frames. ECMWF and CAMS use the exact regional OM source grid recorded in each immutable `ready_for_processing.json`; their WebP dimensions and bounds are never inferred from a shared display grid. Lossless RGBA WebP encoding and scalar/vector contracts are shared by both servers. Current markers and product manifests include the renderer Git revision, so a production code upgrade rebuilds the same OM release instead of incorrectly skipping it.

## Runtime

`read_variable_grid_series` decodes ordinary GFS, ECMWF and CAMS source dependencies once for the complete 121-frame output window, then quantizes and writes them in bounded six-frame encoding blocks. The dependency-heavy GFS `weather_code` source group is read in six-frame blocks so cloud, precipitation, snow and instability grids are not resident for the complete dense output grid at the same time. ECMWF uses its smaller native OM grid and keeps the complete series in one read, avoiding repeated reconstruction of retained runs and their temporal interpolation grids.

Each production invocation runs in a transient systemd service with a bounded memory ceiling and no swap allocation: 1536 MiB for GFS/CAMS and 1792 MiB for ECMWF IFS 0.25, whose full 121-frame weather-code derivation has a higher measured peak. The default CPU quota is 150% and the validated production open-file limit is retained. `OM_WEBP_MEMORY_MAX` and `OM_WEBP_CPU_QUOTA` are explicit operational overrides. The runner fails closed if the memory guard cannot be established, so a renderer regression is terminated without exhausting the production host. Two workers are used by default for GFS, ECMWF and CAMS; `OM_WEBP_WORKERS` can override the deployment default.
